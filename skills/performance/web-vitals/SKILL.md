...
1. **Measure field data, not just the lab.** A Lighthouse run is reproducible
...
3. **Fix CLS by reserving space.** Cumulative Layout Shift comes from images
   with no dimensions, injected banners and ads, and late-swapping fonts. Set
   explicit width and height or `aspect-ratio`, reserve slots for dynamic
...
5. **Attribute each regression before fixing it.** Use the Performance panel and
   the `web-vitals` attribution build to name the exact element, script, or
