...
a time: each needs the other to exist first. Cycles creep in when a helper
reaches back to its caller for a single convenience, and they compound until a
whole subsystem is one tangled unit. Breaking them is mechanical once you can
see them, so the first task is making the cycle visible.
...
4. **Invert the accidental back-reference.** If a low-level module calls up into
   ...
   dependency instead of importing upward. It depends on a parameter, not a
   module.
5. **Move shared code down.** When two peers both need a helper and reach
   through each other to get it, relocate that helper into a lower module
   both import cleanly. The peers stop touching each other entirely.
6. **Split the god module.** A file caught in many cycles is usually doing
   several jobs. Break it along its responsibilities so each piece sits at one
   level and imports only downward.

## Checks

- Does `madge --circular` report zero cycles, and does CI fail on a new one?
- Can you compile or import each module from the former loop entirely on its
  own?
