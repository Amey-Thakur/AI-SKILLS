...
description: Write an Amazon six-pager, a narrative memo read in silence at the start of a meeting so a decision rests on reasoning rather than a presenter's charisma. Use when running an operating, strategy, or investment review that must reach a real decision.
---

# Six-pager narrative

Amazon traded slide decks for a six-page prose memo because bullets let a
presenter hide thin logic behind delivery. Full sentences force the connective
reasoning that a bullet lets you skip, and the gaps show. The meeting opens with
everyone reading in silence, so the room reacts to the argument on the page, not
to how confidently it was pitched.
...
6. **Collect comments in the margins, then debate the hard parts.** Reviewers
   ...
   the reasoning, not to perform it.

## Checks

- Does the memo make an ask, or does it stop at description?
- Can every quantitative claim in the narrative be traced to a labeled appendix
  exhibit?
- Would the argument survive being read cold by someone who never heard you talk
  about it?
